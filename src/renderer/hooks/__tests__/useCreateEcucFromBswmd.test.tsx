...
describe('useCreateEcucFromBswmd — Sprint 14 Task 8 hook', () => {
  it('generates, writes, and adds documents for picked modules (happy path)', async () => {
    ...
    expect(writeArg.files[0]!.filePath).toBe('D:/proj/ecuc/Can_EcucValues.arxml');
    ...
    expect(after.documentPaths).toContain('D:/proj/ecuc/Can_EcucValues.arxml');
  ...
  it('rolls back partial writes on batch failure', async () => {
    ...
    expect(deleteSpy).toHaveBeenCalledWith({ filePath: 'D:/proj/ecuc/Can_EcucValues.arxml' });
    ...
    expect(response?.failed).toEqual([
      { filePath: 'D:/proj/ecuc/CanIf_EcucValues.arxml', message: 'EACCES' },
    ]);
  });
  ...
  it('correctly identifies active document after successful create', async () => {
    ...
    const after = useArxmlStore.getState();
    expect(after.activeDocumentPath).toBe('D:/proj/ecuc/Can_EcucValues.arxml');
    ...
    expect(after.dirtyPaths.has('D:/proj/ecuc/Can_EcucValues.arxml')).toBe(false);
  });
