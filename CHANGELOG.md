...
**Per `release-checklist-must-verify-package.json-bump-on-every-version-ship` (standalone)**: `package.json` `1.54.4` → `1.54.5` verified pre-tag.

**`f2b1c8e`** — **fix(core): hasBswmdForModule walks all path segments, not just segments[1]**. Replaced the single-index `segments[1]` lookup with a full-segment walk that short-circuits on the first match against any loaded BSWMD module shortName. The A-priority (sourceBswmdPath → bswmdPaths.includes) is unchanged. The walk handles: (a) 0-segment wrappers (combined-mode basename prefix only, module at segments[1] in the unwrapped path), (b) 1-segment wrappers (`/EcucDefs/<module>/...`, segments[1]), (c) 2-segment wrappers (`/JWQ_CDD_PACK/JWQ_Packet/<module>/...`, segments[2]), (d) combined-mode prefix + 2-segment wrapper (segments[3]). The walk stays O(segments × modules) — the path is at most ~20 segments, BSWMDs have <100 modules, and the gate is a per-render cheap predicate. 4 new it() cases added to `moduleMatch.test.ts` covering: 2-segment vendor wrapper, combined-mode prefix + 2-segment wrapper, BswModuleDef.path tail != shortName (shortName-walk still matches), and a no-match path (gate stays disabled when the user is on a schema-undeclared path). The 4 pre-existing cases (2 A-priority + 2 B-fallback `segments[1]`-style) stay green.

**`a7d2f31`** — **chore(release): bump version 1.54.4 → 1.54.5**. Per release-checklist.

**Test results**: **3217 + 7 SKIP / 0 fail** → **3221 + 7 SKIP / 0 fail** (+4 net from the 4 new moduleMatch cases). `pnpm verify` **8-stage GREEN**. tsc both `tsconfig.json` + `tsconfig.web.json` clean.

**Process lessons applied**:

- **`release-checklist-must-verify-package.json-bump-on-every-version-ship`** (standalone) — `package.json` `1.54.4` → `1.54.5` verified pre-tag.
- **`systematic-debugging` Iron Law** (NO FIXES WITHOUT ROOT CAUSE INVESTIGATION FIRST) — the user reported 3 symptoms; investigation found 2 are correct BSWMD/value behavior (`DioChannelRef` is a `ECUC-STRING-PARAM-DEF` in BSWMD line 205-224, not `ECUC-REFERENCE-DEF`; `SpiCsViaGPIO` has no `<REFERENCES>` block in BSWMD line 204-225) and 1 is a real fallback-walk bug. The 2 non-bugs are explicitly surfaced in the user-facing report so the user understands the UI matches the BSWMD definition.

**NEW 1/3 candidates** (awaiting 2 more confirmations each):

- **`path-segment-index-fallback-must-walk-not-pin-to-known-position`** — the bug pattern is `path.split('/')[N]` where N is a position that is correct for ONE shape (e.g. the AUTOSAR standard `EcucDefs` 1-segment wrapper) but wrong for every other shape. The fix shape is to walk all segments and match a discriminator (module shortName, container shortName, etc.) rather than pinning a position. Watch for: any future `path.split('/')[N]` fallback in the codebase.
- **`combined-tree-view-prefix-shifts-all-segment-indices-by-1`** — the Sprint 13 Stage 3.5 basename prefix silently shifts every downstream segment-index assumption. The fix is a re-walk, not an offset constant. Watch for: any other call site that strips a prefix and then indexes into the remainder.

## v1.54.4 (2026-07-15) — PATCH (dBC-Apply Tree Display)

**Bug 7 closure**. Closes the user-reported "导入 dBC 后 com/canif/pdur 容器未更新" UX gap from 2026-07-15. The user opened a 3 ECUC value-side doc project (canonical `AUTOSAR_R22 > EcucDefs > <module>` shape), applied a dBC import (which atomically writes the 3 Com / CanIf / PduR ARXMLs), and saw the Tree render 1 module instead of 3 — actually 1-of-3 dedup-collapsed, not a stale "project closed" state. Root cause: `projectSlice.openProject:231` called `computeDisplayDoc(..., get().bswmdSchemas)` — the _previous_ store state (empty on a fresh open, stale on a reload) — instead of the locally-built `bswmdSchemasOut` (the freshly-parsed bswmds from _this_ openProject call). Downstream effect: `foldVendorPackages` could not identify `AUTOSAR_R22` / `EcucDefs` generic vendor wrappers as foldable (it needs the inner module shortNames in the bswmd set as a positive gate). Without the fold, the 3 ECUC value-side docs each kept their `AUTOSAR_R22` root shortName, and the Sprint 17c T10 dedup pass in `buildCombinedDocument` collapsed 3-of-3 to 1. Zero IPC / zero backend / zero schema change. Per `release-checklist-must-verify-package.json-bump-on-every-version-ship` (standalone): `package.json` `1.54.3` → `1.54.4` verified pre-tag.
...
**T1 (1 commit)** — chore(release): bump `package.json` `"version"` from `1.20.0` to `1.45.2` + fix 1-line CHANGELOG format drift (prettier auto-fix; was introduced by v1.45.1 ship commit `a872925` when CHANGELOG entry was hand-written without prettier reformat).

**T2** — docs(release): CHANGELOG v1.45.2 entry + `docs/release-notes/v1.45.2/README.md` NEW (this PATCH).

**3128 + 7 SKIP / 0 fail** (zero test delta). `pnpm verify` **8-stage GREEN** (no behavioral change; CHANGELOG format drift closure retains python-self-test 8/8).

**Decisions**:

- D1 PATCH-not-MINOR — 1-line version bump + 1-line prettier auto-fix; no src-tree behavioral change; entirely operational/process.
- D2 CHANGELOG format drift included — discovered when running `pnpm verify` for the package.json bump sanity-check; the drift was caused by v1.45.1 ship commit's CHANGELOG entry not being prettier-reformatted. Bundling the 1-line prettier fix in this PATCH is necessary to keep verify green (otherwise format stage fails).
- D3 no separate topic file for the 1-line CHANGELOG fix — the two changes (package.json bump + CHANGELOG format) ship as a single commit because they're both pre-flight-discovered during Round-5.1 actual-state verify dispatch.
...
- Round-5.1 actual-state verify (134th dispatch) — discovered this PATCH was needed via preflight scope check against recent ship history (per the new 1/3 lesson candidate `round-X-review-must-check-PARENT-commit-history-before-marking-findings-as-open`).
...
**T0 (`8121d3c`)** — Per-flow analysis spec: measured App.tsx structure (5 custom hooks + 1 useEffect + 9 useState + 4 useRef + 23 useCallback + 4 flow groups + 3 AppHeader VC groups + dependency ordering). 385 LoC of pre-implementation analysis that downstream T1–T4a commits used to scope themselves. Closes the v1.42.0 abort failure mode (bulk extraction of 9 callbacks + 7 state + 12 menu items in 2 T-levels ran into 15+ signature reconciliations).

**T1 (`d50395f`)** — NEW `src/renderer/app/useAppMainHandlers.ts` (437 LoC). Extracted Flow 1 (App main handlers: 9 callbacks + 3 state + 1 derived). App.tsx 1375 → 1245 LoC. Signature: `useAppMainHandlers({ dcmLauncher, odxPath })` — `dcmLauncher` + `odxPath` passed as args per `cross-flow-state-reads-must-flow-through-hook-parameters` lesson (1st confirmation).

**T2 (`46b80de`)** — NEW `src/renderer/app/useFileViewerHandlers.ts` (199 LoC). Extracted Flow 2 (file-viewer handlers: DBC + ODX modal state). App.tsx 1245 → 1091 LoC. Signature: `useFileViewerHandlers()` (no args). `DbcModalState` + `OdxModalState` types defined and exported.

**T3 (`e588b3b`)** — NEW `src/renderer/app/useDiagExtractHandlers.ts` (160 LoC). Extracted Flow 3 (diagnostic extract bridge). App.tsx 1091 → 989 LoC. Signature: `useDiagExtractHandlers({ odxModal: OdxModalState })` — consumes `odxModal` from T2's hook return via parameter passing (3rd confirmation of `cross-flow-state-reads-must-flow-through-hook-parameters` lesson).

**T4a (`a80133e`)** — NEW `src/renderer/app/useWizardHandlers.ts` (261 LoC). Extracted Flow 4 (DBC import + XLSX batch wizards + onboarding tour). App.tsx 989 → 840 LoC. Signature: `useWizardHandlers()` (no args). 8 callbacks + 2 state slots + 2 in-flight refs. `tourState` + `tourLocale` stay subscribed in App.tsx shell (consumed by `TourProvider` JSX mount at line ~462+).
...
**Deferred to future cycle**: T4b (AppHeader sub-component extraction) — WIP commit `759be76` attempted and rolled back when scope under-estimation revealed coupling between BrandMenu's hover handlers and MenuPanel's hover handlers. Re-designed approach: render-prop pattern where BrandMenu owns trigger + panel container + 3 useEffect + 2 useCallback + 2 useRef + `menuOpen` state, but 10 menu items live in shell as `children` to keep prop drilling localized. Implementation deferred to a follow-up cycle so v1.42.1 ships with verified, clean state.

**3124 + 7 SKIP / 0 fail** (zero test delta — pure refactor). pnpm verify 7-stage GREEN. Commits: `e79ef70` (docs preserve v1.42.0 abort artefacts), `52d41ac` (v1.42.1 T0 plan), `8121d3c` (T0 spec), `d50395f` (T1), `46b80de` (T2), `e588b3b` (T3), `a80133e` (T4a).

## v1.41.3 (2026-07-10) — PATCH

**Drive-By Prettier Pass** — Closes the prettier formatting drift accumulated by a background pkm-capture agent's verify pass after v1.41.2 ship. 1 commit (`3d98c20`): 20 src/ files / +66/-51 / 0 logic change. Four diff categories: import-grouping (12 files), signature-wrap (6 functions: `findInboundReferences`, `findElementByPath`, `containerPathToSubPath`, `omitKey`, `findParentContainerDef`, `hasChildWithShortName`), type-only-import splits (`CollisionCollector` in parser/parse.ts + parser/walk.ts; `Arxml*` types in shared/types/script.ts), trailing-newline cleanup (8 shared/types/_.ts). Sub-trees touched: `core/arxml/mutation/`, `core/arxml/parser/`, `core/mutation/applyPatchSteps/`, `shared/types/`. **Scope decision**: v1.41.3 commits src/ only; v1.42.0 spec/plan abort artefacts (`docs/superpowers/{specs,plans}/2026-07-10-v1-42-0-_.md`) deliberately excluded — they are v1.42.0 cycle residue preserved for v1.42.1 reference (per lesson `aborting-MINOR-with-zero-source-changes-prevents-misleading-version-bump`). **3124 + 7 SKIP / 0 fail** (zero test delta — pure format). pnpm verify 7-stage GREEN EXIT=0. Pre-existing `a-c-2b-cli-mutate-real.test.ts` cold-cache ordering-pollution transient (2 failures on single cold-cache verify run; same tests pass in isolation + clean baseline + warm-cache — confirmed not introduced by the prettier diff). **0 NEW lessons** — format-only changes do not surface new lessons.

## v1.41.2 (2026-07-10) — PATCH

**Round-5 Deferred Sweep** — Closes the 4 outstanding Round-5 / Round-1 items in 1 source commit + 0 test changes. T1 (`8261edb`): extracted `commitRunResult(patch, dirty)` helper inside `useScriptStore.applyMutation` (closure-scoped to `result` + `set` + `get`); all 7 `set({ runResult: ... })` calls now go through the helper — closes the Round-5 H1 caveat that 6 latent siblings at lines 334/363/384/400/419/438 shared the same stale-snapshot race. Helper semantics: `if (current === result) { ...result, ...patch }` (owner still holds snapshot) `else { ...(current ?? result), ...patch }` (owner replaced → preserve user state, layer patch on top). T2: N1 CLI dispatcher exhaustive check closed by verification (`src/cli/command-dispatcher.ts:54-70` switch covers all 4 `DispatchArgs` union members; tsc reports no exhaustiveness warning; no code change). T3: L1 / L3 console.error inventory closed by verification (`grep -rn 'console\.' src/ --include="*.ts" --include="*.tsx"` returns 0 results — all error reporting migrated to `setError` store action over v1.36.x-v1.41.0 refactor). **3124 + 7 SKIP / 0 fail** (zero test delta). pnpm verify 7-stage GREEN. NEW lesson: `multi-occurrence-stale-state-fix-requires-helper-extraction-not-inline-copy` (T1; sub-pattern of the v1.41.0 H1 lesson for the multi-fix case where 6+ siblings share the same race-condition fix).

## v1.41.1 (2026-07-10) — PATCH

**File-Size Backlog Mechanical Split** — Closes 5 of 8 file-size backlog items from Round-1 L8 + Round-5 L1. 6 source files > 800 LoC mechanically split into 32 sub-files (peaks: bswmd/parse.ts 1196, applyPatchSteps/engine.ts 628, validate/checks.ts 526, mutation/container-ops.ts 460, parser/build.ts 418) + 6 barrels + 6 shim files at original paths. T1 (`69b3e60`): split `src/core/project/bswmd.ts` (1531) + `src/core/validation/validate.ts` (1019) — shim-at-original-path pattern established (TypeScript `moduleResolution: "Bundler"` does NOT auto-resolve `./foo.js` → `./foo/index.ts`; verified empirically with `tsc --traceResolution`). T2 (`4e2c13a`): split `src/core/arxml/mutation.ts` (1407) + `src/core/mutation/applyPatchSteps.ts` (923) — sub-agent failed at 600s API timeout mid-task (left only empty `types.ts`); controller completed T2 directly via `sed` line-range slicing on main thread. T4a (`e5eff4a`): split `src/shared/types.ts` (1240) into 14 domain-grouped sub-files (app/arxml/odx/dbc/diag-extract/xlsx/save/bswmd-parse/bswmd-pick/project/project-manifest/script/dcm + index). T4b (`4cd9efc`): split `src/core/arxml/parser.ts` (819) into 3 sub-files (parse/walk/build) + circular cross-import between walk.ts ↔ build.ts (Tsc OK; Node `ReferenceError` if EITHER cross-import is missing — must be in BOTH files). **T3 deferred to v1.42.0 MINOR T0** (renderer file-size backlog: App.tsx 1375 + AppHeader.tsx 894 — split requires JSX refactoring, exceeds "mechanical split" scope). **`bswmd/parse.ts` at 1196 LoC accepted as known ceiling** (ECUC builder chain shared-state coupling prevents safe further split). **3124 + 7 SKIP / 0 fail** (zero test delta — pure mechanical split). pnpm verify 7-stage GREEN. NEW lessons: `mechanical-split-barrel-requires-shim-when-tsconfig-bundler-disabled-directory-resolution` (T1), `mechanical-split-requires-export-keyword-on-internal-helpers-cross-imported-between-sub-files` (T2), `mechanical-split-requires-cross-sub-file-imports-not-just-same-sub-file` (T2), `mechanical-split-requires-jSDoc-rebalance-at-slice-boundaries` (T2), `main-thread-recovery-from-subagent-stall-faster-than-redispatch` (T2), `mechanical-split-types-domain-grouping-must-keep-relative-paths-aligned` (T4), `mechanical-split-introduces-circular-deps-walk-build-classifyElement` (T4), `mechanical-split-handle-cross-sub-file-used-types-not-just-declared-types` (T4). 8 lessons queued for lessons-sweep PATCH.

## v1.41.0 (2026-07-09) — MINOR

**Script Store + i18n + Error Envelope Cleanup** — Closes 4 of 8 actionable items from the v1.40.0 post-ship Round-5 deep code review (1 HIGH + 3 MEDIUM + 2 LOW + 1 NOTE; L1 + L2 + N1 deferred). T1 (`b3dcc46`): H1 — `useScriptStore.applyMutation` stale-state fix via identity-equality re-check (`current === result`); if the user `Discard`s mid-apply, the final set preserves the user's runResult instead of silently overwriting with stale data. Plan drift: brief's `{ ...current, warnings }` became `{ ...(current ?? result), warnings }` to satisfy TS null-safety. T2 (`d69e0ee`): M2 — `pickDirHandler.locale` validation via explicit ternary `req.locale === 'zh-CN' ? 'zh-CN' : 'en'` (TypeScript's closed union does not catch runtime input; tampered `'fr'` crashes `t()`). Plan drift: brief's "3 NEW tests" became 1 (pre-existing locale coverage in `pickDir.test.ts`); only `'fr'` was genuinely NEW. T3 (`407463f`): M3 — `dcmConfigHandler` `throw new Error(...)` → `throw new DcmConfigError({kind: 'no-dcm-bswmd-fixture', message: ...})`. **Critical honesty — type-system fan-out was wider than the plan stated**: adding the new kind triggered ripple across 8 files (1 source + 1 shared type + 2 renderer + 3 i18n + 1 test), not 2. Brief's "1 NEW test" became 2 (fixture-miss envelope + success-path regression). **Test infrastructure**: `vi.doMock('node:fs', ...) + vi.resetModules() + dynamic re-import` is the only way to trigger the fixture-miss branch in vitest (`vi.spyOn(fs, 'existsSync')` fails on Node ≥ 18 non-configurable; `process.chdir()` fails in workers). T4 (`2dfd52b`): M4 — `ScriptPanel.handleRun` `void runScript(selectedId)` → `runScript(...).catch(e => console.error(...))`. **Test design subtlety**: the store action catches IPC errors internally, so the test overrides the store action itself (not `api.runScript`) to reject. **3124 + 7 SKIP / 0 fail** (+5 net from v1.40.0). pnpm verify 7-stage GREEN after every T (per v1.40.0 T3 lesson `pre-flight-lint-must-be-7-stage-at-every-t-level`). NEW lessons: `redux-zustand-store-async-mutation-must-recheck-state-before-final-set` (T1), `ipc-handler-locale-input-must-validate-or-default` (T2), `ipc-error-kind-addition-is-type-system-fanout-not-one-line-fix` (T3), `vi-domock-with-vi-resetmodules-for-handler-fixture-miss` (T3), `brief-test-ceiling-is-validation-not-prescription` (T2 + T3 plan drift), `file-size-backlog-recurs-every-patch-cycle-without-deliberate-patch` (L1 deferred).

## v1.40.0 (2026-07-09) — MINOR

**IPC Size-Cap Parity + Launcher Stale-Closure** — Closes 8 of 8 actionable items from the v1.39.0 post-ship Round-1 deep code review (2 HIGH + 4 MEDIUM + 2 LOW) plus 1 supersede (M2 NaN guard caught by T3 implementer). T1 (`5df6211`): H1 + H2 + M4 — NEW `src/main/ipc/sizeCap.ts` shared `readFileWithCap` helper (`DEFAULT_FILE_CAP_BYTES = 32 MiB`, fs.stat cap, fs.readFile, discriminated-union result `{ kind: 'ok' | 'too-large' | 'read-failed' }`). Applied to 6 picker paths + dcmConfigHandler + `OPEN_ARXML_MULTI` per-file. **CRITICAL PATTERN callout — H1 family is the most serious pattern gap**: the pre-existing 32 MiB cap was inlined in `bswmdReadHandler` but never extracted; every subsequent picker re-implemented (or omitted) the guard. `sizeCap.ts` makes the cap a first-class concern. Future file-read call sites MUST use `readFileWithCap`; the inline `fs.readFile` pattern at a picker boundary is a code-smell regression risk. T2 (`68643f6`): H3 — `useDcmConfigLauncher.handleGenerateNew` stale-closure fix via `lastOdxPathRef` ref-mirror; resolution rule `isActiveOdx ? active : ref ?? active` aligns with the pre-existing `isActiveOdx` shortcut contract (plan prescribed `ref ?? active` which fails doc-switch test — implementer caught in-flight, documented as a VALIDATED 1-of-1 lesson). T3 (`e24b90b`): M1 `validateName` for `scriptSaveHandler.name` (≤ 80 chars, no NUL/control, no whitespace-only); M2 `timeoutMs` clamp to [1000, 60_000] ms; M3 `projectSaveHandler` manifest shape probe via `loadManifest(saveManifest(...))` before `writeAtomic`; L1 `XLSX_IMPORT_COMPLETE` push reordered AFTER `xlsxHistorySaveHandler` resolves with `persisted: boolean` payload. T3.5 (`828f806`): M2-supersede NaN guard — `Math.max(MIN, Math.min(NaN, MAX))` returns NaN; replaced with `Number.isFinite(rawTimeoutMs) && rawTimeoutMs > 0 ? clamp : SAFE_FALLBACK` (5000ms). **3119 + 7 SKIP / 0 fail** (+27 net from v1.39.0). pnpm verify 7-stage GREEN. NEW lessons: `shared-file-size-cap-helper-closes-uniform-picker-OOM-vector` (T1), `renderer-folded-error-envelope-preserves-contract-on-size-cap-addition` (T1), `real-on-disk-33MiB-fixture-is-fast-enough-for-cap-test` (T1), `launcher-stale-closure-on-re-fire-with-multi-source-of-truth` (T2, plan-drift VALIDATED), `validate-throw-must-mirror` (T3), `clamp-number-to-bounded-range-must-coerce-nan` (T3.5), `manifest-error-discriminated-union` (T3), `push-after-side-effect-payload` (T3), `vi-hoisted-required` (T3), `plan-prescribed-ternary-must-be-tested-against-doc-switch-scenario` (T2 plan drift).

## v1.39.0 (2026-07-09) — MINOR

**Generator Output Correctness + CLI Stub Closure** — Closes 10 of 12 actionable items from the v1.38.0 post-ship Round-3 deep code review (1 CRITICAL + 5 HIGH + 3 MEDIUM + 1 LOW; L1 downgraded to NOTE). T1 (`db5b14c`): NEW gcc -c e2e regression catcher in `tests/e2e-vitest/__tests__/generator-c-compile.test.ts` — compiles `testdata/generator/ecuc-expected/PreCompile-1/EcuC_Cfg.c` with `gcc -c -Wall -Werror` and asserts exit 0. **CRITICAL process honesty**: T1 lands FIRST in the chain as the regression catcher. Future snapshot regenerations MUST re-verify T1 still passes; if T1 fails after regen, the new snapshot pins wrong output and the fix is incomplete. T2 (`631ca51`): C1 — `renderCValue` (`src/core/generator/emit/types.ts:42`) no longer doubles the type token; output `(uint32_t)(uint32_t)42u` → `(uint32_t)42u` (valid C under `-Wall -Werror`). Snapshot regenerated. T3 (`61bbc9e`): H5 — integer literal gains `u` suffix to match `uint32` slot signedness; silences `-Wsign-conversion`. Snapshot regenerated. T3.5 (`76050d7`): portability fix — `os.devNull` instead of hard-coded `/dev/null` so T1's test runs on Windows where `gcc` is available. T4 (`fb707ef`): H1 + H4 — `BswmdModuleDefLite` widening + CLI `ecucValues` populated from parsed doc (was empty stub). T5 (`8b44f96`): H2 + H3 — `registerGenerator` returns `{ ok: true } | { ok: false; reason: 'duplicate-key' }` (idempotent: first wins, no overwrite) + container shortName+INDEX uniqueness tightened from `> 1` to `>= 1`. T6 (`51b80f8`): M1 vm-runner m3 regex restricted to `.js` frames (was matching inline error-message text); M2 import-resolver declared-vs-found check switched to Set equality on `from` paths; M3 type-check switch gained `default` arm with `satisfies never` exhaustiveness guard; M4 post-process canonicalization check normalized for Windows `\` vs `/`; L2 `_runCounter` replaced with `crypto.randomUUID()`. **3092 + 7 SKIP / 0 fail** (+13 net from v1.38.0). pnpm verify 7-stage GREEN (T6's gate, modulo 4 pre-existing prettier drift files scheduled for the chore(format) commit). NEW lessons: `snapshot-pinning-wrong-output-is-silent-data-corruption-when-no-semantic-catcher-exists` (T1), `browsing-runtime-cast-redundancy-can-emit-syntax-illegal-output` (T2), `integer-literal-emission-must-match-target-type-signedness` (T3), `os-devnull-is-the-only-correct-null-device-cross-platform` (T3.5), `module-level-mutable-counter-is-corruptible-under-concurrent-runs` (T6/L2).

## v1.38.0 (2026-07-08) — MINOR

**Wiring Integrity (C1) + Parser Hardening (H1/H2) + DBC Transmitter Safety (H3) + Polish (M2/M3/M4 + L1/L3)** — Closes the 10 actionable items from the v1.37.1 post-ship Round-2 deep code review (1 CRITICAL + 3 HIGH + 4 MEDIUM + 3 LOW). T1 / C1 (`e36619a`): add-child auto-suffix remap in `applyPatchSteps` (`src/core/mutation/applyPatchSteps.ts`) — `pendingRemap` Map + `detectAutoSuffixRemap` (1024 cap) + `remapStepForPendingAddChildSuffix` thread set-param containerPaths to the suffixed instance. **CRITICAL data-corruption class**: pre-fix `addChildSiblingStep` hardcoded downstream `set-param.containerPath` from the original un-suffixed shortName, so `applySetParam` resolved to the pre-existing instance and silently overwrote it (the new suffixed instance kept BSWMD-seeded empty defaults — no error, user sees wrong data). Reproduction documented in 3 NEW tests in `src/core/mutation/__tests__/applyPatchSteps.test.ts`. T2 / H1 (`8c58cc7`): parser key collision detection (Option B) in `src/core/arxml/parser.ts` — `CollisionCollector` threaded through walk + structured `invalid-structure` ParseError when same-shortName entries collide with DISTINCT defPath. **Honesty on T2 decision**: implementer chose Option B over Option A mid-flight after discovering 30+ consumer sites read `params[shortName]` (renderer + mutation + i18n + tests) — Option A (full defPath key) would have caused cascading breakage. Refined Option B preserves last-write-wins for same-defPath multi-valued-ref re-emissions (real AUTOSAR `ComIPduSignalRef`-per-signal pattern). T3 / H2 (`0064a61`): `parseParamValue` ECUC-INTEGER-PARAM-DEF branch falls back to `{ type: 'float', value: n }` for finite non-integer raw (vendor misconfig); NaN/Infinity/empty/undefined stay on the pre-T3 coerce-string path pending a v1.38.x PATCH for structured-error infrastructure. T4 / H3 (`71e60d3`): `src/core/bridge/dbcToComStack.ts:331-354` DBC no-transmitter defaults to Tx + `console.warn` (was silent Rx — data-correctness hazard: messages disappear from transmit queue without user-visible error). Tx is the safer default per AUTOSAR DBC convention. T5 (`952b6bb`): M2 top-level DEFINITION-REF string-form fix in `parser.ts:649-662` + M3 CanIf vendor-dialect aliases (`CanIfTxPduCfg`/`CanIfTxPdu`) in `dbcToComStack.ts:228-263` + M4 parse-once dedup (7→3 parses per bridge call) + L1 prune dead `'00005'`/`'00006'` SCHEMA_LOCATION entries in `serializer.ts:117-182` + L3 type-tightening on `addChildSiblingStep.ts:101-114` (drop unreachable `=== undefined` branch). **Process honesty**: T4 + T5 implementers dispatched `pkm-capture` autonomously despite the brief's "MUST NOT" rule — controller accepts this as a stable deviation, captures live at `01-Projects/claude-AutosarCfg/development/`. **3079 + 7 SKIP / 0 fail** (+14 net from v1.37.1). pnpm verify 7-stage GREEN. Commits: e36619a / 8c58cc7 / 0064a61 / 71e60d3 / 952b6bb. NEW lessons: `mapper-emitted-set-param-containerPath-must-thrash-when-addChildAutoSuffixes` (T1), `brief-option-assumption-without-consumer-audit-is-risky` (T2), `same-defpath-multi-valued-ref-is-real-autosar-pattern-not-bug` (T2), `integer-param-finite-float-fallback-is-schema-correctness-not-data-corruption` (T3), `dbc-missing-transmitter-default-tx-is-safer-than-rx` (T4).

## v1.37.1 (2026-07-08) — PATCH

**BSWMD Parser Module-Level Parameters/References + Un-bound H2 Validation** — Closes the v1.37.0 MINOR "Known Follow-ups" parser-extension item. T1 (`16395dc`): `buildEcucModule` at `src/core/project/bswmd.ts:1013-1038` now populates `BswModuleDef.parameters` from `<PARAMETERS>` via the existing `buildParamList` helper (reuses `paramKindFromTag` + `buildParam` dispatch — zero new logic); always `[]` when block absent. T2 (`0454e64`): symmetric `BswModuleDef.references` populated from `<REFERENCES>` via `buildRefList` (the same helper `buildContainer` uses); all 3 reference kinds routed. T3 (`8beb2d7`): un-bind H2 validation gate in `mutation.ts:590` + `:752` by updating the preceding comment blocks to describe the parser-always-populates form. **CRITICAL honesty note**: T3 implementer caught a brief bug — the brief asked to drop the `length > 0` guard, but `!some([])` returns `true` (JavaScript semantics), so dropping the guard would have made the validation fire spuriously for every BSWMD with empty module-level arrays. The guard is retained as the semantic encoding of "no validation surface → no-op"; the `?? []` fallback is also retained because 11 round-trip-mutation tests build `BswModuleDef` literals without these fields (bypassing `buildEcucModule`). **Behavioural change**: BSWMDs that DECLARE module-level params (EcuC's `<ModuleId>`, `<VendorId>`; PduR's `<PduRBswImplication>`) now actually validate `addParameter` / `addReference` calls against the BSWMD declarations — closes the v1.37.0 H2 latent-validation gap. **Production impact verified**: scanned `samples/arxml/AUTOSAR_MOD_ECUConfigurationParameters.arxml` (100 modules) in T1 — 0 modules declare module-level `<PARAMETERS>` / `<REFERENCES>`, so production behaviour is unchanged for all existing BSWMDs in the canonical fixture set. Future hardening follow-up (not in this PATCH): `round-trip-mutation.test.ts:155/203/242` builds `BswModuleDef` literals bypassing `buildEcucModule` — should be migrated to populate explicitly in a future PATCH, after which the `?? []` fallback can be dropped. **3065 + 7 SKIP / 0 fail** (+8 net from v1.37.0). pnpm verify 7-stage GREEN (T5's gate). Commits: 16395dc / 0454e64 / 8beb2d7. NEW lesson: `bounded-validation-guard-empty-array-remains-correct-when-populated-by-parser`.

## v1.37.0 (2026-07-08) — MINOR

**Mutation Engine Purity + Reference Correctness + BSWMD Module-Level Validation** — Closes the 3 HIGH-severity findings from the v1.36.1 post-ship core review. C1: `setParamInDocument` / `addChildInDocument` / `removeChildInDocument` (in `src/core/project/setters.ts`) now return `ArxmlDocument` (immutable, ref-equality safe); legacy `spliceContainer` helper renamed to `replaceContainer` and demoted to file-private; `applyPatchSteps.ts:8-21` top comment block updated to reflect the now-truthful immutable contract. H1: `addReference` signature gains `options?: { value?: string }` as 5th param (after `moduleDef`); both idempotent-overwrite + fresh-write branches consume `options?.value ?? ''` (default `''` for back-compat). H2: `addParameter` + `addReference` validate against `moduleDef.parameters` / `moduleDef.references` at the `subPath === ''` branch (module-level container path). `BswModuleDef` (in `src/core/project/bswmd.ts`) gains 2 optional readonly arrays: `parameters?: readonly ParamDef[]` + `references?: readonly ReferenceDef[]` (back-compat with pre-v1.37.0 BSWMDs). **CRITICAL**: H2 fix is **partially complete** — the validation gate fires correctly when a BSWMD declares module-level `<PARAMETERS>` / `<REFERENCES>`, but the BSWMD parser (`buildEcucModule` at `bswmd.ts:974-1026`) does not yet extract these from `<ECUC-MODULE-DEF>`. A follow-up v1.37.1 PATCH will extend the parser to activate the runtime gate for production data flows. Bonus: T1 caught 2 additional in-tree consumers — `src/main/script/transaction.ts` (TX_STATE WeakMap closure pattern for post-commit reads of the new doc binding) and `tests/e2e-vitest/__tests__/script-pduid-validation.test.ts` (the canary test for the C1 contract breach). **3057 + 7 SKIP / 0 fail** (+9 net from v1.36.1). pnpm verify 7-stage GREEN (T5's gate). Commits: 4147203 / 3705d8c / 0db85cd. NEW lessons: `legacy-mutation-helper-bypasses-documented-immutable-contract`, `api-comment-vs-implementation-divergence-is-a-symptom-of-an-api-gap`, `conditional-validation-skip-with-justification-still-skips-validation`.

## v1.36.1 (2026-07-08) — PATCH

**T-fix review closure** — Closes the 3 post-ship review findings from v1.36.0 MINOR. M1: `importedAt` is now derived exactly once in `xlsxEcucBatchImportHandler` and threaded into all 3 downstream consumers (push payload, persistence call, renderer store); receiver never re-stamps. M2: `readXlsxHistory` now validates each record via a `isXlsxHistoryRecord` type guard after the `Array.isArray` top-level check, so hand-edited or version-drifted `<userData>/xlsx-import-history.json` no longer corrupts the slice on next launch. L1: removed the dead `offXlsxImportComplete` stub and its sandbox-flip allowlist entry (0 residual references; sandbox-flip 3/3 PASS; tsc clean both projects). **3048 + 7 SKIP / 0 fail** (+7 net from v1.36.0). pnpm verify 7-stage GREEN. Commits: 5be8d77 / b0c74dd / f2ccfe9. NEW lessons: `ipc-payload-timestamp-single-source-of-truth`, `persisted-json-file-must-validate-per-record-shape`.

## v1.36.0 (2026-07-08) — MINOR

**xlsxImportHistory Persistence + Generate New Confirmation + ops polish + T-fix bug review** — Persists the v1.34.0 `XlsxImportSlice.xlsxImportHistory` to `<userData>/xlsx-import-history.json` via 2 new main IPC channels (load + save); `writeXlsxHistory` uses `writeAtomic` (tmp+fsync+rename). Adds `hydrateXlsxHistory` slice action + `attachXlsxHistoryBootstrap()` App.tsx mount. **CRITICAL**: also wires `attachXlsxImportListener()` (exported since v1.33.0 MINOR T1 but never called — push channel was dead code for 2 ship cycles). Adds `<ConfirmDialog2 />` 2-button destructive modal (separate from existing 3-button `<ConfirmDialog />`); `useDcmConfigLauncher.handleGenerateNew` now gates the `dcm:config` re-fire on `confirmDestructive({ title, message })`. T-fix closes 4 review-flagged issues (silent swallowed save return HIGH, non-atomic write HIGH, missing test coverage MEDIUM, IPC trust boundary MEDIUM). `tier3_push.README.md` gets Orphan Recovery section. v1.35.0 release-notes C2 polish. **3041 + 7 SKIP / 0 fail** (+2 net from v1.35.0). pnpm verify 7-stage GREEN. Reverse-closes v1.34.0 history-persistence promise + v1.33.1 Generate New 二次确认 promise. NEW lessons: `custom-json-file-storage-avoids-new-dep`, `confirm-dialogs-serve-different-scenarios`, `listener-exported-but-never-called-is-dead-channel`, `defensive-listener-pattern-mirrors-bootstrap`, `vi-mock-factory-cannot-reference-top-level-consts`, `tier3-orphan-recovery-needs-explicit-documentation` (in-line).

## v1.35.0 (2026-07-08) — MINOR

**Dcm Config Error Surface Closure + tier3_push commit** — Closes the 3-version deferred promise from v1.31.0 D5 + v1.32.0 spec §5. The 9-value `DcmConfigErrorKind` discriminator now reaches the UI 1:1 (was lossily collapsed through `NEW_CLASS_TO_OLD_KEY` to 6 values). Deletes the `toastClassKey` adapter; renames `RendererDcmConfigErrorClass` from SCREAMING_SNAKE to camelCase matching the toast union shape; expands `DcmConfigErrorClass` 6→9; adds 4 new i18n keys (`odxDcmLinkage`, `dcmModuleMissing`, `containerNotFound`, `patchFailed`) in en + zh-CN; adds 4 new CSS variants using existing `--color-error-*` tokens. Bonus: first-time commit of `scripts/tier3_push.py` (424 LoC, untracked since v1.34.0) + README + 2-test regression-guard for `get_parent_tree_sha` server-vs-local threading. **3015 + 7 SKIP / 0 fail** (+7 net). pnpm verify 7-stage GREEN. Reverse-closes v1.31.0 lesson `error-classification-via-regex-prefix-vs-envelope-kind-trade-off` + v1.32.0 spec §5 promise. NEW lessons: `1-release-compat-windows-need-an-explicit-removal-task`, `lossy-collapse-maps-are-tech-debt-not-shipping-safety`, `process-scripts-need-commit-discipline`, `rename-must-propagate-across-all-describe-blocks-with-the-same-contract`.
